...
import { reCalculateCartesian, headingPitchRollToVector, rayEarthIntersection } from '../../utils/CesiumMath';
  ...
  methods: {
    ...
    globeView () {
      if (this.sceneModeToogleTool.mode === '2D') {
        ...
        let destination = new Cesium.Cartesian3(copy.destination.x, copy.destination.y, copy.destination.z);
        let direction = headingPitchRollToVector(destination,
